						...
						User.findOne({id :  req.token.sid}).exec(function(err, user){
							...
							Property.findOne({property_id: property_id }).exec(function(err, property_details){
								...
								if(user.company_id ==  property_details.company_id ){
									...
									qry = "select property_masteritem_link.* from property_masteritem_link where property_masteritem_link.type='DEFAULT' and property_masteritem_link.status=1 and property_masteritem_link.property_id ='" + property_id + "' order by property_masteritem_link.priority, property_masteritem_link.option ";
									...
									// Property_masteritem_link.query(qry, function(err, prop_tempalte){
									//
									// 	return res.json({status: 1, room: prop_tempalte});
									//
									// });
